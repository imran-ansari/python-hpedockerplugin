...
class HPE3ParMediator(object):
    ...
    def _build_createfshare_kwargs(self, protocol, fpg, vfs, fstore, readonly,
                                   sharedir, proto_opts, comment,
                                   client_ip=None):
        ...
        if protocol == 'nfs':
            ...
            if self._config.hpe3par_require_cifs_ip:
                if client_ip:
    ...
    def _create_share(self, share_details):
        proto_opts = share_details['protocolOpts']
        readonly = share_details['readonly']
        share_id = share_details['id']
        share_name = share_details['shareName']
        # share_name = self.ensure_prefix(share_id,
        #                                 readonly=readonly)

        share_dir = share_details['shareDir']
        # if not (share_dir or self.hpe3par_fstore_per_share):
        if not share_details['shareDir']:
            share_dir = share_name

        fpg_name = next(iter(share_details['fpg'][0]))
        vfs_name = share_details['vfs']
        fstore_name = share_details['fstore']
        protocol = share_details['protocol']
        ...
        try:
            ...
            if readonly and protocol == 'nfs':
                # For NFS, RO is a 2nd 3PAR share pointing to same share_dir
    ...
    def create_share(self, share_details):
        ...
        protocol = self.ensure_supported_protocol(share_details['protocol'])
        share = self._create_share(share_details)
    ...
    def delete_share(self, share):
        ...
        fpg = next(iter(share['fpg'][0]))
        vfs = share['vfs']
        fstore = share['fstore']

        # If share-dir was not specified on share creation then
        # the share is at file-store level. Deleting file-store
        # will remove the entire hierarchy under it and free up
        # the space
        if not share['shareDir']:
        ...
        if fstore == share_name:
            ...
            try:
                ...
                self._delete_file_tree(
                    share_name, share['shareDir'], protocol, fpg, vfs,
                    fstore, share['vfsIP'])
    ...
    def _delete_file_tree(self, share_name, share_dir, protocol, fpg, vfs,
                          fstore, share_ip):

        def _get_mount_path():
            prefix = '/mnt'
            if share_dir:
                path = '/'.join([prefix, share_dir, 'del_share'])
            else:
                path = '/'.join([prefix, share_name, 'del_share'])
            return path

        mount_location = _get_mount_path()
        ...
        share_dir_path = mount_location + "/%s" % share_name

        # Delete the share from the super share.
        self._delete_share_directory(share_dir_path)
    ...
    def _create_super_share(self, protocol, fpg, vfs, fstore, readonly=False):
        sharedir = ''
        extra_specs = {}
        comment = 'Docker super share used to delete nested shares.'
        my_ip = self._host_config.my_ip
        ...
        if protocol == 'nfs':
            createfshare_kwargs['clientip'] = self._host_config.my_ip
        else:
            createfshare_kwargs['allowip'] = self._host_config.my_ip
